...
module CanvasAPI
  ...
  def self.api_call(path, user_config, all_pages=false)
    ...
    #insecure should be set as true if SSL is required on the LMS
    canvas = Canvas::API.new(:host => host, :token => user_config.access_token, :insecure => true)
    begin
...
module OAuthConfig
  def self.oauth_config(org, domain)
    ...
    oauth_config ||= ExternalConfig.first(:config_type => 'canvas_oss_oauth')
